<!DOCTYPE html>
<html lang="id">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Pengajuan Surat Desa</title>

    <!-- CSS -->
    <link rel="stylesheet" href="{{ asset('asset_halaman_desa/css/form_penyuratan.css') }}">

    <!-- Boxicons CSS -->
    <link href="https://unpkg.com/boxicons@2.1.2/css/boxicons.min.css" rel="stylesheet">
</head>

<body>
    <section class="container forms">
        <div class="form register">
            <div class="form-content">
                <header>Pengajuan Surat Pengantar Nikah</header>
                <form action="#" method="#">
                    <br>
                    <div id="#"> <!-- Isi Form -->
                        <div class="mb-3">
                            <label class="form-label">Nama Pria</label>
                            <input type="text" class="form-control" name="#" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Nama Wanita</label>
                            <input type="text" class="form-control" name="#" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">NIK Pria</label>
                            <input type="text" class="form-control" name="#" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">NIK Wanita</label>
                            <input type="text" class="form-control" name="#" required>
                        </div>                        
                        <div class="mb-3">
                            <label class="form-label">No KK Pria</label>
                            <input type="text" class="form-control" name="#" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">No KK Wanita</label>
                            <input type="text" class="form-control" name="#" required>
                        </div>                    
                        <div class="mb-3">
                            <label class="form-label">Surat Ketarangan Status Perkawinan</label>
                            <input type="file" class="form-control" name="#" required>
                        </div>
                    </div> <!-- End -->

                    <button type="submit" class="btn-submit">Ajukan Surat</button>
                </form>
            </div>
        </div>
    </section>

</body>

</html>